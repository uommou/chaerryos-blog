...
{   
    ...
    int lim_y = maps.size();
    int lim_x = maps[0].size();
    vector<vector<int>> distance (lim_y, vector<int> (lim_x, -1)); // 거리체크
    ...
    while(!q.empty()) {
        ...
        for (int i=0; i<4; i++){
            ...
            if (nx < 0 || ny < 0 || nx >= lim_x || ny >= lim_y) continue; // 범위 초과
            if (maps[ny][nx] == 0 || distance[ny][nx] != -1) continue;
    ...
    answer = distance[lim_y-1][lim_x-1];
    return answer;
}
```